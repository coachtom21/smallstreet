...
if (!defined('CPM_TWILIO_PLUGIN_URL')) {
    define('CPM_TWILIO_PLUGIN_URL', get_template_directory_uri() . '/cpm-tawilio');
}

/**
 * Get user country code (placeholder - you can customize this)
 */
function ct_get_user_country_code()
{
    // For now, return NP (Nepal) - you can customize this logic
    return 'NP';
}
...
function ct_check_discord_membership($discord_user_id)
{
    if (empty($discord_user_id)) {
        return false; // No Discord ID
    }

    // For now, return false to simulate pending status
    // You can implement actual Discord API check later
    return false;
}
...
function ct_validate_twilio_otp()
{
    ...
    if ($verification_check->status == "approved") {
        ...
        if (
            ...
            // Return success with XP message
            wp_send_json_success([
                "valid_otp",
                wp_create_nonce('ct_user_signin'),
                "xp_awarded",
                "🎉 SUCCESS! OTP Verified and XP Awarded! Check your account for details."
            ]);
            ...
            // Return success but no XP message
            wp_send_json_success([
                "valid_otp",
                wp_create_nonce('ct_user_signin'),
                "no_xp",
                "⚠️ OTP Verified but No XP Awarded. To earn XP, scan QR code and verify OTP from that page!"
            ]);
        }
    } else {
...
function ct_save_xp_after_otp_verification($phone_number)
{
    ...
    if (!$user) {
        ...
        if (!empty($users)) {
            $user = $users[0];
            error_log("User found by get_users: YES (ID: {$user->ID})");
        } else {
    ...
    error_log("Account Type: " . ($is_seller ? 'Seller' : 'Buyer'));
    ...
    if ($is_seller) {
        ...
        error_log("Seller meta update result: " . ($update_result ? 'SUCCESS' : 'FAILED'));
        error_log("Total seller transactions: " . count($seller_metas));
        ...
        error_log("Existing buyer meta: " . (is_array($buyer_meta) ? count($buyer_meta) . ' transactions' : 'Not array'));

        // Assign empty array if meta is empty
        $buyer_metas = !empty($buyer_meta) ? $buyer_meta : [];
        if (!is_array($buyer_metas)) {
            $buyer_metas = [];
        }
        ...
        error_log("Buyer meta update result: " . ($update_result ? 'SUCCESS' : 'FAILED'));
        error_log("Total buyer transactions: " . count($buyer_metas));
    ...
    error_log("=== XP SAVING FUNCTION COMPLETED ===");
    return true;
}
